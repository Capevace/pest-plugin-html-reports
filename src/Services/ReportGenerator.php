...
{
	public function generate(TestResult $testResult): array
	{
		$resultJsonData = [
			'counts' => [
		...
		$isErrored = fn(string $className, string $methodName): Errored|null => $first($errorEvents, $className, $methodName);
		$isFailed = fn(string $className, string $methodName): Failed|null => $first($failedEvents, $className, $methodName);
